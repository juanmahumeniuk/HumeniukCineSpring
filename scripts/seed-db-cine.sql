...
--
-- Copia canónica en classpath: src/main/resources/db/seed-db-cine.sql
-- La app la ejecuta sola al iniciar si no hay películas (ver DatabaseSeeder).
--
-- Uso manual (app detenida):
--   mysql -u jota -p db_cine < scripts/seed-db-cine.sql
--
-- Si ya tenés datos y querés empezar de cero, descomentá el bloque TRUNCATE abajo.

SET NAMES utf8mb4;
SET FOREIGN_KEY_CHECKS = 0;

-- ---------------------------------------------------------------------------
-- Limpieza opcional (descomentar si querés reemplazar todo el contenido)
-- ---------------------------------------------------------------------------
-- TRUNCATE TABLE venta_cliente;
-- TRUNCATE TABLE venta_funcion;
-- TRUNCATE TABLE compra_proveedor;
-- TRUNCATE TABLE compra_insumo;
-- TRUNCATE TABLE cine_pelicula;
-- TRUNCATE TABLE empleado_cine;
-- TRUNCATE TABLE entrada;
-- TRUNCATE TABLE venta;
-- TRUNCATE TABLE compra;
-- TRUNCATE TABLE funcion;
-- TRUNCATE TABLE sala_vip;
-- TRUNCATE TABLE sala;
-- TRUNCATE TABLE cliente_vip;
...
-- Sincronizado con src/main/resources/db/seed-db-cine.sql
-- ---------------------------------------------------------------------------
...
-- Cartelera por cine (todas las películas en los 3, ajustá si querés diferenciar)
...
INSERT INTO funcion (id, horario, pelicula_id, sala_id) VALUES
  ( 1, '2026-05-18 14:00:00',  1,  1),  -- Star Wars IV
  ( 2, '2026-05-18 17:30:00',  2,  1),  -- Star Wars V
  ( 3, '2026-05-18 20:45:00',  9,  3),  -- Dark Knight (VIP)
  ( 4, '2026-05-18 15:15:00', 13,  2),  -- Avengers
  ( 5, '2026-05-18 19:00:00', 19,  2),  -- Jurassic Park
  ( 6, '2026-05-19 16:00:00',  5,  4),  -- Star Wars VII
  ( 7, '2026-05-19 21:00:00', 11,  6),  -- The Batman (VIP)
  ( 8, '2026-05-19 18:30:00', 14,  5),  -- Spider-Man
  ( 9, '2026-05-20 13:30:00', 18,  7),  -- LOTR
  (10, '2026-05-20 22:00:00', 30,  9),  -- Inception (VIP)
  (11, '2026-05-20 17:00:00',  8,  8),  -- Batman Begins
  (12, '2026-05-21 20:00:00', 23,  4);  -- Mission Impossible
...
-- Ajustar AUTO_INCREMENT para próximos inserts vía API
...
SELECT 'Seed completado.' AS mensaje,
       ...
       (SELECT COUNT(*) FROM cine) AS cines,
       (SELECT COUNT(*) FROM funcion) AS funciones;
